...
class UserService {
    ...
    async getUserById(userId) {
        const users = await this.db.query(`SELECT id, username, email, full_name, role, permissions, is_active
       FROM users
     ...
     */
    determinePermissionLevel(user, taskAssignee, taskType, orderCreatorId) {
        // 1. ADMIN 拥有所有权限
        if (user.role === 'ADMIN') {
            return 'APPROVE';
        }
     ...
     */
    async getApprovers() {
        const users = await this.db.query(`SELECT id, username, email, full_name, role, permissions
       FROM users
