...
exports.DataDrivenDemo = DataDrivenDemo;
 ...
 */
const VIEW_MODE_SCHEMA = {
    schemaId: 'dept-manager-approval-form',
    schemaName: '部门经理审批',
    taskId: 'task-001',
    nodeId: 'node-approval-level1',
    permissionLevel: permission_types_1.PermissionLevel.VIEW,
    fields: [
        {
            id: 'orderNumber',
            name: 'orderNumber',
...
const EDIT_MODE_SCHEMA = {
    ...VIEW_MODE_SCHEMA,
    permissionLevel: permission_types_1.PermissionLevel.EDIT,
    fields: [
        {
...
function DataDrivenForm({ schema }) {
    const [formData, setFormData] = (0, react_1.useState)({});
    // 数据驱动渲染 - 完全根据schema渲染
    return (<div className="data-driven-form">
      {/* 1. 权限指示器 */}
      ...
      <div className="fields-container">
        {schema.fields.map((field) => (<DynamicFieldRenderer key={field.id} field={field} value={formData[field.id] ?? field.value} onChange={(value) => setFormData({ ...formData, [field.id]: value })}/>))}
      </div>

      {/* 3. 动态按钮渲染 - 完全由actions数组决定 */}
      <div className="actions-container">
        {schema.actions.map((action) => (<button key={action.id} className={`btn btn-${action.id}`} onClick={() => handleAction(action)} disabled={schema.permissionLevel === permission_types_1.PermissionLevel.VIEW}>
...
function DynamicFieldRenderer({ field, value, onChange, }) {
    ...
    return (<div className="field-wrapper">
      ...
      {field.type === 'text' && (<input type="text" value={String(value ?? '')} disabled={field.readonly} // 由后端决定
         onChange={(e) => onChange(e.target.value)} className="field-input"/>)}
      
      {field.type === 'textarea' && (<textarea value={String(value ?? '')} disabled={field.readonly} // 由后端决定
         onChange={(e) => onChange(e.target.value)} className="field-textarea"/>)}
      
      {field.type === 'enum' && (<select value={String(value ?? '')} disabled={field.readonly} // 由后端决定
         onChange={(e) => onChange(e.target.value)} className="field-select">
...
function DataDrivenDemo() {
    const [currentSchema, setCurrentSchema] = (0, react_1.useState)(VIEW_MODE_SCHEMA);
 ...
 */
async function fetchDynamicSchema(orderId, taskId) {
 ...
 * 前端使用示例
 */
function RealWorldUsage({ orderId }) {
    ...
    if (loading)
        return <div>加载中...</div>;
    if (!schema)
        return <button onClick={loadSchema}>加载表单</button>;
    return <DataDrivenForm schema={schema}/>;
}
//# sourceMappingURL=data-driven-demo.js.map