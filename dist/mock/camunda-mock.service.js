...
exports.camundaMockService = exports.CamundaMockService = void 0;
const sales_order_types_1 = require("../domains/sales/models/sales-order.types");
/**
 * Camunda模拟数据服务
 */
class CamundaMockService {
    constructor(config = {}) {
        this.config = {
     ...
     * 生成模拟的流程实例
     */
    generateProcessInstances(orders) {
        return orders.map(order => this.createProcessInstance(order));
    }
    /**
     * 生成模拟的审批历史
     */
    generateApprovalHistory(processInstances) {
     ...
     * 生成模拟的外部任务
     */
    generateExternalTasks(processInstances) {
        const tasks = [];
    ...
    createRandomOrder(index) {
        ...
        const order = {
            ...
            status: sales_order_types_1.SalesOrderStatus.PENDING_APPROVAL,
        ...
        if (this.config.includeCustomFields) {
            order.customFields = this.createRandomCustomFields(order);
        }
        return order;
    }
    /**
     ...
     * 获取随机审批级别
     */
    getRandomApprovalLevel() {
        const levels = [sales_order_types_1.ApprovalLevel.LEVEL_1, sales_order_types_1.ApprovalLevel.LEVEL_2, sales_order_types_1.ApprovalLevel.LEVEL_3];
        return levels[Math.floor(Math.random() * levels.length)];
    }
    /**
     * 创建流程实例
     */
    createProcessInstance(order) {
        return {
            ...
            variables: {
                ...
                // 添加自定义字段
                ...(order.customFields && { customFields: order.customFields }),
            },
     ...
     * 创建审批历史
     */
    createApprovalHistory(instance) {
        const history = [];
        // 模拟审批历史
        ...
        for (let i = 0; i < Math.floor(Math.random() * 3) + 1; i++) {
            history.push({
                ...
                approvalLevel: sales_order_types_1.ApprovalLevel[`LEVEL_${i + 1}`],
     ...
     * 创建外部任务
     */
    createExternalTask(processInstanceId, index) {
        return {
     ...
     * 创建随机自定义字段
     */
    createRandomCustomFields(order) {
        return {
...
exports.CamundaMockService = CamundaMockService;
// 导出默认实例
exports.camundaMockService = new CamundaMockService();
exports.default = CamundaMockService;
//# sourceMappingURL=camunda-mock.service.js.map