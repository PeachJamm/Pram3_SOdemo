...
import { UserService, PermissionLevel } from '../../database/services/user.service';
import { FormRendererService } from '../../dynamic-forms/form-renderer.service';
import { Camunda8Client, Camunda8TasklistClient, Camunda8IntegrationService } from '../../orchestration/camunda8-client';
...
interface TaskInfo {
  ...
  processDefinitionKey?: string; // 用于从Camunda获取表单
...
interface FormRenderResponse {
  ...
  data?: {
    ...
    processFlow: {
      ...
      bpmnXml: string;  // BPMN XML 字符串
      steps: ProcessStep[];  // 从 Camunda 实时获取的状态
    };
    // 注意：approvalHistory 已移到 GET /api/forms/tasks/by-order/:orderId API
  };
  error?: string;
}
...
export class FormController {
  ...
  private camundaTasklist: Camunda8TasklistClient;
  private camundaIntegration: Camunda8IntegrationService;
  ...
  constructor() {
    ...
    this.formRenderer = new FormRendererService();
    this.camundaClient = new Camunda8Client({
      gatewayAddress: 'localhost:26500',
      plaintext: true,
    });
    this.camundaTasklist = new Camunda8TasklistClient('http://localhost:8088/tasklist');
    this.camundaIntegration = new Camunda8IntegrationService();
    this.setupRoutes();
  }
  ...
  private async renderForm(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      let formResult: { schema: any; version?: number } | null = null;
      try {
      ...
      // 4. 判定权限级别
      const permissionLevel = this.userService.determinePermissionLevel(
        user,
        taskInfo.assignee,
        formSchema.id,
        taskInfo.variables.createdBy
      );
      ...
      const response: FormRenderResponse = {
        success: true,
        data: {
   ...
   * }
   */
  private async submitForm(req: Request, res: Response): Promise<void> {
    const { taskId } = req.params;
    const { userId, variables } = req.body as { userId: string; variables: Record<string, any> };
    ...
    try {
      ...
      try {
        ...
        await this.db.execute(
          `INSERT INTO approval_history (id, sales_order_id, approver_id, task_id,
        ...
        // 更新订单状态
        await this.db.execute(
          `UPDATE sales_orders SET status = ?, updated_at = ? 
           WHERE id = ? OR process_instance_key = ?`,
          [newStatus, new Date().toISOString(), orderId, taskInfo.processInstanceKey]
        );
   ...
   */
  private async getFormSchema(req: Request, res: Response): Promise<void> {
   ...
   */
  private async getTaskByOrderId(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const order = await this.db.queryOne<{ process_instance_key: string; order_number: string }>(
        `SELECT process_instance_key, order_number FROM sales_orders WHERE id = ?`,
        [orderId]
      );
      ...
      if (tasks.length === 0) {
        ...
        // 构建流程导航步骤（基于流程状态）
        const processFlow = await this.buildProcessFlowForOrder(
          order.process_instance_key,
          processStatus.currentElementId || 'unknown',
          null // 没有当前任务
        );
      ...
      const taskDetailsPromises = tasks.map(async (task: any) => {
      ...
      const mappedTasks = tasksWithDetails.map((task: any) => {
   ...
   */
  private createMockTaskForUser(user: any, orderId: string, orderNumber: string): any {
    // 根据用户角色确定任务类型
    let taskName = '订单验证';
    let formKey = 'order-validation';
   ...
   * 从数据库获取完整订单变量（包括产品明细）
   */
  private async getOrderVariablesFromDB(processInstanceKey: string): Promise<Record<string, any>> {
    try {
      // 1. 获取订单主表数据
      ...
      const items = await this.db.query<any>(
        `SELECT soi.*, p.name as product_name, p.product_code as product_code
   ...
   * 构建产品明细表格字符串
   */
  private buildProductLinesTable(productLines: any[]): string {
   ...
   * 计算预期审批级别
   */
  private calculateExpectedApprovalLevel(totalAmount: number, customerTier: string): string {
   ...
   */
  private async getProcessInstanceStatus(processInstanceKey: string): Promise<{
   ...
   * 根据元素ID获取名称
   */
  private mapElementIdToName(elementId?: string): string {
    if (!elementId) return '流程处理中';
   ...
   * 获取非 UserTask 的显示名称
   */
  private getNonUserTaskName(processStatus: { state: string; currentElementName?: string }): string {
   ...
   */
  private async getProcessFlowStatus(taskInfo: TaskInfo): Promise<{
    processInstanceKey: string;
    processStatus: 'ACTIVE' | 'COMPLETED' | 'CANCELED';
    currentStepId: string;
    bpmnXml: string;
   ...
   */
  private async buildProcessFlowForOrder(
    processInstanceKey: string,
    currentElementId: string,
   ...
   */
  private async getFlowNodeStatus(
    processInstanceKey: string,
    currentElementId: string,
    taskInfo: TaskInfo
  ): Promise<ProcessStep[]> {
    ...
    if (!orderId) {
      try {
        const order = await this.db.queryOne<{ id: string }>(
          `SELECT id FROM sales_orders WHERE process_instance_key = ?`,
          [processInstanceKey]
        );
   ...
   */
  private async buildStepsForOrder(
    processInstanceKey: string,
    currentElementId: string,
    currentTask: any | null
  ): Promise<ProcessStep[]> {
    ...
    // 2. 转换为 steps（包含类型）
    const steps: ProcessStep[] = userTasks.map((task: any) => {
    ...
    // 4. 插入网关节点
    const gatewayNodes: ProcessStep[] = [
      {
   ...
   */
  private mapTaskStateToStatus(state: string, isCurrent: boolean): 'COMPLETED' | 'CURRENT' | 'PENDING' {
    switch (state) {
      case 'COMPLETED':
        return 'COMPLETED';
      case 'CREATED':
  ...
  private inferGatewayStatus(
    ...
    // 根据前后任务状态推断
    const beforeTask = userTasks.find(t => 
      gatewayId === 'gateway-approval-path' ? t.id === 'task-order-validation' : 
  ...
  private mergeStepsInOrder(
    userTasks: ProcessStep[],
    dmnNodes: ProcessStep[],
    gatewayNodes: ProcessStep[]
  ): ProcessStep[] {
    // 定义流程顺序
    const processOrder = [
      'start-event',
    ...
    return processOrder
      ...
      .filter((n): n is ProcessStep => n !== undefined);
  }

  /**
   * 回退步骤（当 Camunda API 不可用时）
   */
  private buildFallbackSteps(currentElementId: string, currentTask: any | null): ProcessStep[] {
    const allSteps: ProcessStep[] = [
   ...
   * 将 Camunda 状态映射为前端状态
   */
  private mapCamundaStateToStatus(camundaState: string, nodeId: string, currentElementId: string): 'COMPLETED' | 'CURRENT' | 'PENDING' {
    switch (camundaState) {
      case 'COMPLETED':
        return 'COMPLETED';
      case 'ACTIVE':
   ...
   * 从审批历史获取已完成的步骤
   */
  private async getCompletedStepsFromHistory(orderId: string): Promise<{
    stepId: string;
    stepName: string;
    assignee: string;
    completedAt: string;
    ...
    try {
      const history = await this.db.query<any>(
        `SELECT ah.*, u.username, u.full_name 
   ...
   * 将 action 映射到步骤 ID
   */
  private mapActionToStepId(action: string): string {
    const mapping: Record<string, string> = {
      'SUBMIT': 'start-event',
   ...
   * 将 action 映射到步骤名称
   */
  private mapActionToStepName(action: string): string {
    const mapping: Record<string, string> = {
      'SUBMIT': '订单提交',
      'VALIDATE': '订单验证',
      'APPROVE_SM': '销售经理审批',
   ...
   * 获取审批历史（用于页面右侧时间线）
   */
  private async getApprovalHistory(orderId: string): Promise<{
    id: string;
    stepName: string;
    approverName: string;
    action: string;
   ...
   */
  private fillFormValues(components: any[] | undefined, variables: Record<string, any>): any[] {
   ...
   */
  private async getTaskInfoFromCamunda(taskId: string): Promise<TaskInfo | null> {
   ...
   */
  private async loadFormSchema(taskInfo: TaskInfo): Promise<{ schema: any; version?: number } | null> {
