...
export class FormRendererService {
   ...
   */
  async loadFormSchema(formKey: string): Promise<FormSchema | null> {
  ...
  renderForm(
    ...
    permissionLevel: PermissionLevel,
    variables: Record<string, any> = {},
    taskInfo: { taskId: string; taskType: string; assignee: string | null }
  ): RenderedForm {
    // 递归处理组件，应用权限过滤
  ...
  private processComponent(
    ...
    // 复制组件，避免修改原始数据
    const processed: FormComponent = { ...component };
    ...
    if (component.components && component.components.length > 0) {
      processed.components = this.processComponents(
   ...
   */
  private applyDefaultPermission(component: FormComponent, permissionLevel: PermissionLevel): void {
    ...
    switch (permissionLevel) {
      ...
      case 'EDIT':
        ...
        // 审批相关字段默认只读
        if (
          component.key?.includes('approval') ||
   ...
   * 评估条件显示规则
   */
  private evaluateConditional(conditional: any, variables: Record<string, any>): boolean {
  ...
  extractFormFields(
    ...
    const extractFromComponents = (components: FormComponent[] | undefined) => {
