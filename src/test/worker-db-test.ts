...
import { CamundaMockService } from '../mock/camunda-mock.service';
...
class TestRunner {
  ...
  private mockService: CamundaMockService;
  ...
  constructor() {
    ...
    this.mockService = new CamundaMockService({ orderCount: 3 });
   ...
   * 测试客户数据加载
   */
  async testCustomerService(): Promise<TestResult> {
   ...
   * 测试产品数据加载
   */
  async testProductService(): Promise<TestResult> {
   ...
   * 测试订单创建草稿
   */
  async testOrderDraftCreation(): Promise<TestResult> {
   ...
   * 测试订单保存到数据库
   */
  async testOrderSave(): Promise<TestResult> {
    return this.runTest('订单保存测试', async () => {
      const items = [{ productId: 'prod-001', quantity: 2 }];
      const draft = await this.orderService.createOrderDraft('cust-001', items);
      if (!draft) throw new Error('Failed to create order draft');
   ...
   * 测试模拟数据生成
   */
  async testMockDataGeneration(): Promise<TestResult> {
   ...
   * 模拟 Job Worker 处理
   */
  async testJobWorkerSimulation(): Promise<TestResult> {
   ...
   * 模拟财务处理 Worker
   */
  private async simulateFinanceWorker(order: any): Promise<any> {
   ...
   * 模拟库存预留 Worker
   */
  private async simulateInventoryWorker(order: any): Promise<any> {
   ...
   * 模拟通知 Worker
   */
  private async simulateNotificationWorker(order: any): Promise<any> {
   ...
   * 测试完整的订单流程（模拟）
   */
  async testCompleteOrderFlow(): Promise<TestResult> {
   ...
   * 测试订单查询
   */
  async testOrderQuery(): Promise<TestResult> {
